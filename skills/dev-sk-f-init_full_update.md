...
Perform a comprehensive research pass across the entire Flint and rewrite `Mesh/(System) Flint Init.md` from scratch.
...
1. **Research the Flint.** Spawn multiple parallel subagents to gather the full current state:
   - Read `flint.toml` for the configuration and the references of the Flint
   ...
   - Check the references of the Flint (codebases, APIs) and their current state
