...
The first write command makes the folder: `init`, `start`, `sync`, `import`, `add`, or `org set`. `sync.json` is stamped after a run that is `ok` and changed something. A dry run never stamps it. `heal`, `rename`, `dissolve`, and the record upgrade write a backup before they rewrite or remove a box file. A corrupt journal is kept beside the journal as `git-sync-state.json.corrupt-<time>`.
...
- **Owned member** (`mode = "own"`; the default for a Git URL). The box owns the Flint. The source is a Git URL. The local sync clones it into the box as `(<Type>) <name>`, for example `(Flint) NUU Mesh`. Each machine that has the box clones the same member.
- **Reference member** (`mode = "reference"`; the default for `registry:<name>`). The Flint stays at its path on this machine. The local sync finds it in the roster (`~/.nuucognition/places.json`) and wires it. It never clones, copies, or moves it. Every reference spelling works: the name, the folder form, and the address (`@org/flint/<slug>`, `@/flint/<slug>`). When the roster of this machine does not have the Flint, the member is `blocked` with one next command: `flint register <path>` when the Flint is on disk, else `flint create <name>`.
- **Required and optional.** `[flints].required` holds the members that the box needs. `[flints].optional` holds the members that some machines cannot reach: a failed clone of an optional member is a warning, not a failure.
...
"On this machine" (the column `PRESENT` of `status`) means: an owned member that is cloned in the box, or a reference member at its roster path.
...
- The local sync never moves or deletes a folder. Undeclared folders stay. `check` reports them.
...
- `--only <names...>` and `--skip <names...>` select members. The repos are synced only in a run with no `--only` and no `--skip`.
- In a box with an org, an owned member with no org or another org is a notice with the next command `flint tinderbox org set <org> --id <uuid> --apply`. The local sync never writes the org of a member.
...
`flint tinderbox git sync` is the transport of the box. It exchanges the history of the box repo and of each member with origin, with the local sync in the middle, in the order of `flint git sync`:

1. **The box:** checkpoint, fetch, integrate. No push yet. A halt of the box stops the run before the members.
...
- `git resolve <member>` fixes one member: it continues the held rebase, or it syncs the member again with `--local` or `--remote` (`--replay` after a remote history rewrite).
- `git publish <url>` sets origin of the box repo, names the branch `main`, commits pending changes, and pushes.
...
Each Tinderbox command renders as `flint sync` and `flint git sync` do: one title line with the status, sections with glyphs, and each block once.

| Status | Meaning | Exit |
|---|---|---|
| `ok` | Each item is done or current | 0 |
| `partial` | Some items failed, and other items are done | 1 |
| `failed` | The run failed | 1 |
| `blocked` | The run cannot continue until the operator acts: a held rebase, a reference member that the roster does not have, a halt of the box | 2 |

- `tinderbox sync` exits as `flint sync` does. `tinderbox git sync`, `git resume`, and `git resolve` exit as `flint git sync` does.
- A refusal (no box, an invalid `tinderbox.toml`, a bad argument) exits 1 with the reason and the next command.
- `check` exits 1 when it has a finding. A required reference member that the roster does not have is a finding.
- A dry run marks nothing `synced` and counts nothing `done`. A blocked member in a dry run is `blocked`, not `failed`.
- Each problem has one `Next` line with one command.
- `--json` prints one JSON value on every path, with `status` and `next`.
...
| `init --from <url>` | Clones the box from its Git URL into the current folder, then runs the local sync. | `--no-open` | 0, 1 |
...
| `remove <name>` | Removes a member from `tinderbox.toml` and the record, strips its wiring, and keeps its folder and its roster row. | `--move-out <dir>` | 0, 1 |
...
| `dissolve` | Backs up and removes `tinderbox.toml` and `tinderbox.json`, removes the roster row, strips the wiring, and keeps `.git`, the member folders, and `.tinderbox/`. | `--dry-run`, `--move-to <dir>`, `--force`, `--yes` | 0, 1 |
...
| `heal` | Repairs the drift that `check` finds (with a backup of `tinderbox.toml`), then runs the local sync of the box. | `--dry-run`, `--yes`, `--json` | 0, 1 |
...
- **The commands that move a folder:** `start` (the current Flint into the new box), `import` (a Flint of the roster into the box; it asks unless `--yes`), `remove --move-out <dir>`, `dissolve --move-to <dir>`, `rename` (the member folder; `--tinderbox` moves the box folder), and `heal` (it gives a member folder its canonical name, and it moves a broken member folder aside as `<folder>.broken-<time>`).
...
- **The commands that write `tinderbox.toml`:** `init`, `start`, `import`, `add`, `remove`, `rename`, `heal`, `repo add`, `repo remove`, `connection add`, and `connection remove`. `heal`, `rename`, and `dissolve` back up the box files to `.tinderbox/backups/` first.
- **Preview first.** Run `--dry-run` first where it exists: `sync`, `heal`, `dissolve`. `org set` without `--apply` is a plan.
- **The commands that write nothing:** `status`, `check`, `git status`, `repo list`, `connection list`, each `--dry-run`, and `org set` without `--apply`.
- **`dissolve` protects work.** It refuses when a member has uncommitted or unpushed work, unless `--force`.
...
- In a box with an org, an owned member with no org or another org is a notice in `sync`, `sync --dry-run`, and `check`, with the next command `flint tinderbox org set <org> --id <uuid> --apply`. No command heals it: the local sync never writes the org of a member.
- `status` shows the org of the box in its title, or `no org (local tier)`.
...
- A folder that holds both `tinderbox.toml` and `flint.toml` is refused.
